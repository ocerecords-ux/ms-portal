...
- **Tabule ve studiích** — obrazovka ve studiu s dnešním programem, poznámkami a tím, co chybí. Nahoře svítí, co právě běží — natáčení i střih; když jede víc věcí naráz, ostatní jsou pod tou hlavní jako „Zároveň“. Tabuli si může otevřít i člověk z týmu pod svým účtem: na kartě uživatele se zaškrtne **Přístup na tabule** a pak ji má rovnou v horní liště pod odkazem **Tabule** (a taky v Můj účet → Tabule ve studiu). Každé studio má **účet tabule** (jméno jako „brno1“ a heslo, bez e-mailu, spravuje se v Administraci → Studia); počítač u obrazovky se jím přihlásí v Chromu a portál rovnou ukáže tabuli toho studia. Do zbytku portálu se tímhle účtem nedostane. V okně vedle programu běží **příběhy z Instagramu** (když žádný není, poslední příspěvky) - účet se připojuje v Administraci → Studia a u každého studia jde okno vypnout.
...
5. **Stav projektu** se během toho posouvá: V přípravě → Natáčíme → Dotočeno → Dokončeno - ke schválení → Schváleno - k fakturaci → Vyfakturováno. Podrobně je to v návodu *Stavy projektu a tlačítko Dotočeno*. Když je projekt ve stavu **Schváleno - k fakturaci** a faktura se klientovi odešle z portálu, **Bruno projekt sám ukončí** (stav Vyfakturováno) a napíše to do kanálu projektu. V jiném stavu (třeba u zálohové faktury během natáčení) se projekt neukončí.
