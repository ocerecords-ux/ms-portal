...
- **Výstupy** — u reklam: seznam toho, co se pod projektem doopravdy vyrábí. Jeden výstup = jedna odevzdaná věc (spot, online voiceover, zkrácená verze) a u každého se zadává jen **název, délka a licence** — plus nenápadně, kdo v něm mluví, takže pod jednou zakázkou může být pět spotů a v každém jiný herec. Režii, hudbu a datum výroby má projekt jednou v záložce Rodný list a dokument si je odtamtud vezme. Zkrácené verze se zakládají tlačítkem **Downcut** u hlavního spotu: ťukne se na délku (30, 20, 6 s) a zbytek dědí po něm, dokud se u nich nevyplní vlastní. **Rodný list se dělá u každého výstupu zvlášť** — rozhoduje typ výstupu, takže pod jedním projektem ho rádiový spot dostane a online voiceover ne. Co klient naklikal v objednávce, sem přijde jako **návrh** (odznak „návrh klienta") a uložením řádku ho produkce potvrdí. Tlačítko **Nabídka z výstupů** (jen kdo vidí doklady) založí rozpracovanou nabídku — jedna položka za výstup, ceny se doplní jen tam, kde je zná ceník. Projekt založený dřív si při prvním otevření udělá jeden výstup ze svých dosavadních údajů.
