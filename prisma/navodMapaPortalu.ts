...
Klientovi reklamy se stavy ukazují jeho řečí: **V přípravě, Plánujeme, Natáčíme**, pak **Ke schválení** (u nás „Dokončeno - ke schválení") a od „Schváleno - k fakturaci" dál už jen **Dokončeno** — fakturace ani opravy do jeho přehledu nepatří. Klient, který u nás dělá **jen reklamy**, má v Projektech **vlastní přehled**: název, stav, herec, typ projektu, licence, datum dokončení, odkaz do složky na Disku a tlačítko **Připomínkovat** (vede do taggeru spotu, kde píše připomínky a spot schvaluje). Klepnutím na licenci se otevře okno s **dokumenty ke stažení** — rodný list u rádiového spotu, licenční list u ostatních reklam. Normostrany se mu neukazují, u reklamy nedávají smysl; firma, která dělá i audioknihy, má přehled jako dosud.

Vedle názvu zakázky svítí dvě malé značky s tím, **jaké doklady u ní visí**: cenovka je nabídka (šedá rozepsaná, oranžová odeslaná, zelená schválená, červená odmítnutá), lístek je faktura (oranžová vystavená a nezaplacená, zelená uhrazená). Co u zakázky není, nesvítí; čísla dokladů jsou v bublince. **Klepnutí na ikonu ukáže náhled** (okno vyskočí hned u ní a zavře se dalším klepnutím kamkoliv do něj) — u nabídky a faktury odběratele, částku s DPH, vystavení, splatnost a stav (po splatnosti svítí červeně), u sluchátek kolik stop je poslechnutých, kolik je zápisů chyb a kdy se na tom naposledy pracovalo. V okně je tlačítko **Otevřít**, které pustí celý doklad nebo přeposlech v novém panelu, ať o přehled nepřijdete. Vidí to jen ten, kdo má na kartě zaškrtnuté **Vidí Banku** — stejná dvojice, co vidí peníze.
...
**Doklady** jsou peníze: nabídky, faktury, upomínky, přijaté výdaje, smlouvy, banka a „Moje firmy" (za kterou firmu doklad vystavujeme, číselné řady, bankovní účty, podpis na faktury). U výdaje se nahraje PDF nebo fotka dokladu (i víc souborů) — údaje se z něj přečtou samy; přílohy i údaje jde kdykoli později doplnit v detailu výdaje. Co se platí **na vícekrát** (smlouva, velká faktura), se v detailu výdaje zapisuje po částech — karta **Úhrady**: kolik odešlo a kdy. Portál z toho drží **kolik je uhrazeno a kolik zbývá doplatit**, v seznamu svítí odznak „Částečně" a QR platba se kreslí už jen na zbytek. Uhrazeno se doklad označí sám, jakmile se částka dorovná.

Když herec po podepsané smlouvě pošle ještě **fakturu** (typicky plátce DPH — na smlouvě je částka bez daně), nezakládá se druhý náklad. U výdaje ze smlouvy je karta **Faktura ke smlouvě**: buď se vybere doklad, který už v portálu leží mezi nezařazenými, nebo se faktura rovnou nahraje. Doklad pak má dvě přílohy (smlouvu i fakturu), v přehledech se počítá **jednou** a částka i QR platba jsou podle faktury, tedy **s DPH**.

Na záložce **Upomínky** se nastavuje, co se děje s fakturou, kterou klient nezaplatil včas. Upomínají se jen faktury **odeslané, nezaplacené a po splatnosti** — koncept ani uhrazená faktura nikdy. Nastavuje se, kolikátý den po splatnosti upomínka odejde (výchozí třetí, desátý a jednadvacátý den, tedy tři upomínky — kolik čísel, tolik upomínek), předmět a text (do textu jdou proměnné jako {cislo}, {castka}, {splatnost}, {dnu}) a komu má chodit skrytá kopie. Pod tím je **náhled e-mailu** přesně tak, jak ho klient uvidí, a seznam faktur po splatnosti s tlačítkem **Poslat upomínku** — to nečeká na ranní úlohu a pošle hned. Dokud není automatické posílání zapnuté, odcházejí upomínky jen tímhle tlačítkem. Píše se tomu, komu šla faktura: kontaktu firmy, a když ho firma nemá, klientovi projektu.

**Kalendář** má vedle studií ještě tři samostatné kalendáře: **Mimo studio** (kdo není), **Porady** (žluté, jen pro pozvané) a **Schůzky** (tyrkysové, společné pro Žůžo-labůžo a produkci). Jinak je to studio a natáčení: kdo kdy natáčí, kdy je studio blokované a nabídky termínů pro herce. Události zapisuje a upravuje produkce a vedoucí pobočky ve svých studiích (zaškrtává se na kartě uživatele – Tomáš Ilavský Brno, Ondřej Černý ml. Praha); ostatní si zapisují Mimo studio. Tlačítko „+ Přidat“ vedle přepínače Den/Týden/Měsíc otevře zápis i bez dvojkliku. Ikonka se čtyřmi rohy vedle šipek přepne kalendář na **celou obrazovku** (zpátky Escapem nebo druhým klepnutím). Vedle nadpisu Kalendář jsou dvě ikony: **výstražný trojúhelník** s počtem konfliktů (co se pere — vaše vlastní vidíte jen vy, z natáčení jen to, kde jste označení) a **hodiny se šipkou** s historií, kdo kdy co v kalendáři změnil. Konflikt, který je schválně, odklepnete v panelu tlačítkem **Je to záměr** — přestane svítit i v liště a dole v panelu zůstane, aby šel vrátit. Jakmile se některá z těch dvou událostí posune, upozornění se vrátí samo.
...
- **Přehledy → Obrat a zisk** (jen admin) — obrat z vydaných faktur, náklady z výdajů a zisk bez DPH v korunách: dlaždice se srovnáním s minulým obdobím, graf po měsících nebo čtvrtletích (najetím přesná čísla, jde přepnout na tabulku), obrat podle klientů, náklady podle kategorií a tabulka projektů. Nahoře výběr roku nebo posledních 12 měsíců, firmy a přepínač Vystaveno/Uhrazeno (podle data dokladu, nebo jen peníze, které opravdu přišly a odešly). V režimu **Uhrazeno** se výdaj placený na vícekrát počítá **po jednotlivých úhradách** ke dni, kdy peníze odešly — ne celý naráz.
...
- **Rezervace studia klienty** — muzikanti a producenti, kteří si u nás bookují studio, mají vlastní kalendář na **/studio**: vidí volno, berou si termíny sami a rezervace platí hned. Svoje události vidí pojmenované, cizí (včetně našeho natáčení) jen jako obsazený čas bez názvů. Zapíná se u každého studia zvlášť v Administrace → Studia, tam se taky posílají pozvánky a odebírá přístup; totéž má pod kalendářem studia i **vedoucí pobočky** (panel Správa rezervací na /studio), takže kvůli pozvánce nemusí do administrace. Kdo nějaké studio s rezervacemi spravuje, má odkaz **Studio** rovnou v horní liště (stejně jako Tabuli) — a když si ho odtud smaže, zůstane smazaný. Rezervace se zapíše rovnou do kalendáře studia jako událost druhu **Rezervace studia**, takže se počítá do obsazenosti i konfliktů a dá se u ní stejně jako u jiné události poznamenat zvukař nebo ji přepsat na jiný druh práce. Návod pro klienty je anglicky, čtou ho na **/studio/napoveda**. Klient si pod svým jménem nastaví údaje a tři upozornění: potvrzení rezervace, změna nebo zrušení termínu z naší strany (posílá se, když s rezervací v kalendáři hneme nebo ji smažeme) a připomínka den předem.
- **Natáčecí plán** — plánování studia a herce.
- **Výstupy** — u reklam: seznam toho, co se pod projektem doopravdy vyrábí. Jeden výstup = jedna odevzdaná věc (spot, online voiceover, zkrácená verze) a celý se zadává **na jednom řádku**: název, kdo v něm mluví, délka a licence. Herci i licence se vybírají v malém okně u tlačítka, délka se píše v sekundách a od minuty výš v minutách (**30s**, **2min.**, **1min.30s**) — napsat jde 90, 1:30 i „1 min 30 s“, portál si to srovná. Režii, hudbu a datum výroby má projekt jednou v záložce Rodný list a dokument si je odtamtud vezme. Tečky na konci řádku nabídnou rodný list, zkrácenou verzi (ťukne se na délku a zbytek dědí po hlavním spotu) a smazání. **Rodný list se dělá u každého výstupu zvlášť** — rozhoduje typ výstupu, takže pod jedním projektem ho rádiový spot dostane a online voiceover ne. Co klient naklikal v objednávce, sem přijde jako **návrh** a uložením řádku ho produkce potvrdí. Tlačítko **Nabídka z výstupů** (jen kdo vidí doklady) založí rozpracovanou nabídku — jedna položka za výstup, ceny se doplní jen tam, kde je zná ceník. Projekt založený dřív si při prvním otevření udělá jeden výstup ze svých dosavadních údajů. Tlačítko **Natáčecí text na Disk** vyrobí ve složce projektu **editovatelný dokument**: jeden za projekt, spoty pod sebou, u každého název, délka a licence a pod tím místo na text — psát se do něj bude rovnou na Disku, portál do hotového dokumentu nesahá a každé další kliknutí udělá nový soubor. Podobu listu si tým píše v Administraci → **Vzory natáčecích textů** (úvod dokumentu a blok spotu, s proměnnými). Název se řídí stopáží a licencí: jakmile se vyplní délka, doplní se do něj sám (**STRABAG - 2min._online**) a při změně se přepíše — do ručně psané části portál nesahá. Vpravo vedle řádků je **náhled** toho listu — celá **A4** naráz, bez posouvání, přesně tak, jak bude vypadat na Disku: fialová hlavička se zeleným logem, pod ní datum poslední úpravy výstupů (ne dnešek — to by se četlo jako den natáčení) a spoty pod sebou bez číslování.
- **Rodný list** — jen u rádiového spotu; PDF se vyrobí z údajů výstupu. K faktuře se přikládají rodné listy všech výstupů, které ho mají mít — když u některého chybí údaje, faktura neodejde a hláška řekne, u kterého výstupu co doplnit.
...
- **Přeposlech** — AudioTagger: poslech nahrávky a zápis chyb s časem a stranou scénáře. Jak daleko to je, hlásí **sluchátka** — v přehledu jako malé kolečko u ikony typu projektu, v detailu jako odznak v hlavičce: **oranžová**, jakmile je v AudioTaggeru aspoň jeden zápis, **zelená**, když je přeposlechnuto komplet. Najetím myší se ukáže, kolik chyb je zapsaných, kolik stop doposlechnutých a kolik procent textu.
...
1. **Objednávka.** Klient ji pošle v portálu (audiokniha nebo reklama). **U reklamy je to průvodce krok za krokem**: název zakázky, co od nás chce (Natáčení voiceoveru / Zvuková postprodukce / Sound design — s ikonami a klidně víc naráz), představa o hlasu, termín a nakonec shrnutí s poznámkou a podklady. Herec i termín se dají přeskočit. Objednané služby přijdou i v mailu o nové objednávce. Vznikne projekt, kanál v chatu a složka na Disku, a nám přijde mail — tlačítko v něm vede **rovnou na detail toho projektu** a poznámka z objednávky je u projektu v záložce Poznámky. U Audiotéky se v objednávce navíc vyplní autor, překladatel a nakladatelství a z nich se sám složí **úvod a závěr audioknihy** (režie vždy Ondřej Černý) — klient ho může upravit nebo přepsat, v detailu projektu je vidět a dá se doladit. Cena v objednávce se počítá ze sazby za normostranu; u firmy, která si **cenu navrhuje sama** (zaškrtávátko na kartě firmy — takhle to má Albatros), se nepočítá nic: pole Počet normostran i Cena zůstávají a klient si je vyplní sám, sazba se u ní vůbec nenastavuje. **U audioknihy se rovnou založí nabídka** — jedna položka „Natáčení a postprodukce audioknihy" a cena z objednávky. Je **rozpracovaná**, takže klientovi sama neodejde: v Dokladech ▸ Nabídky se dá upravit a teprve pak odeslat. Když by se nezaložila (třeba objednávka bez ceny), zůstane v kartě projektu tlačítko **Přidat nabídku** jako dřív.
...
3. **Natáčení.** Produkce sestaví hercovi nabídku termínů, herec si z odkazu vybere, produkce potvrdí — a tím vzniká rezervace studia. **V den první frekvence se projekt sám překlopí na „Natáčíme“** (ráno, podle kalendáře) — díky tomu ho v chatu uvidí i zvukaři, kterým se projekt „V přípravě“ neukazuje. Smlouva s hercem se založí ze šablony a herec ji podepíše taky odkazem.
...
5. **Stav projektu** se během toho posouvá: V přípravě → Natáčíme → Dotočeno → Dokončeno - ke schválení → Schváleno - k fakturaci → Vyfakturováno. Podrobně je to v návodu *Stavy projektu a tlačítko Dotočeno*. **U reklam** fajfku *Dotočeno* nikdo neklikne — den po natáčení (podle kalendáře) ji hercům doplní Bruno sám, aby byli v seznamu zvýraznění. Nechodí u toho žádná zpráva ani nic do chatu a stav reklamy zůstává, kde je. Když je projekt ve stavu **Schváleno - k fakturaci** a faktura se klientovi odešle z portálu, portál se rovnou u faktury **zeptá „Ukončit projekt?"** — tlačítkem Ukončit projekt se stav přehodí na Vyfakturováno (tiše, do chatu se o tom nepíše, je to vidět v historii projektu), tlačítkem Nechat běžet zůstane, kde je. Samo se nic neukončí. V jiném stavu (třeba u zálohové faktury během natáčení) se portál neptá.
...
- **Upomíná faktury po splatnosti** — ve všední dny ráno, podle nastavení v Dokladech ▸ Upomínky. Tutéž upomínku nepošle dvakrát.
