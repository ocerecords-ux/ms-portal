...
- **Výstupy** — u reklam: seznam toho, co se pod projektem doopravdy vyrábí. Co klient naklikal v objednávce, sem přijde jako **návrh** (odznak „návrh klienta") a uložením řádku ho produkce potvrdí. Tlačítko **Nabídka z výstupů** (jen kdo vidí doklady) založí rozpracovanou nabídku — položka za každou službu u každého výstupu, ceny se doplní jen tam, kde je zná ceník. Jeden výstup = jedna odevzdaná věc (spot, online voiceover, zkrácená verze). U každého je délka, co se na něm dělá (natáčení voiceoveru, postprodukce, sound design), **kdo v něm mluví**, hudba, licence a účel užití — takže pod jednou zakázkou může být pět různých spotů, v každém jiný herec. Zkrácené verze se zakládají tlačítkem **Downcut** u hlavního spotu: stačí ťuknout na délku (30, 20, 6 s) a všechno ostatní podědí po něm, dokud se u nich nevyplní vlastní. **Rodný list se dělá u každého výstupu zvlášť** — rozhoduje typ výstupu, takže pod jedním projektem ho rádiový spot dostane a online voiceover ne. Projekt založený dřív si při prvním otevření udělá jeden výstup ze svých dosavadních údajů.
